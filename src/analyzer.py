...
class LearningAnalyzer:
    ...
    def analyze_student_performance(self) -> Dict:
        ...
        student_scores = self.df.groupby('student_id').agg({
            'score': 'mean',
            'activity_type': 'count'
        ...
        return {
            'top_students': (
                student_scores.nlargest(5, 'score').to_dict('index')
            ),
            'performance_distribution': (
                student_scores['performance_level']
                .value_counts()
                .to_dict()
            ),
            'correlation_activity_score': float(
                student_scores['activity_count']
                .corr(student_scores['score'])
            )
        }
    ...
    def analyze_activity_effectiveness(self) -> Dict:
        ...
        activity_scores['effectiveness_rank'] = (
            activity_scores['avg_score'].rank(ascending=False)
        )
    ...
    def generate_recommendations(self) -> List[Dict]:
        ...
        if 'peak_hours' in time_patterns:
            ...
            recommendations.append({
                ...
                'description': (
                    f'Наиболее продуктивные часы: '
                    f'{", ".join(map(str, peak_hours))}:00'
                ),
        ...
        if 'score_stats' in stats:
            ...
            if avg_score < 70:
                recommendations.append({
                    ...
                    'description': (
                        f'Средний балл ({avg_score:.1f}) ниже '
                        f'рекомендуемого уровня'
                    ),
